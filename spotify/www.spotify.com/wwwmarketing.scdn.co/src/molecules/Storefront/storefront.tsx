...
export const Storefront = ({ header = null, icons, labels, currentProductText, errorMessage, errorBanner = false, plans, bundles, bundleHeader, container = null, hasDropShadow = true, additionalCard, eventParams, legalDisclaimer, }) => {
    ...
    }, [icons, paymentIcons, additionalPaymentIcons, labels, device]);
    return (React.createElement(StyledContainer, Object.assign({}, container, eventParamAttributes(eventParams), { "data-current-plan": getCurrentPlan(), "data-storefront-error": errorMessage, "data-payment-icons": !isGPBEligible }),
        header && (React.createElement(React.Fragment, null,
            header.text && (React.createElement(Title, { dangerouslySetInnerHTML: { __html: purify(header.text) } })),
            header.subheader && (React.createElement(Description, { dangerouslySetInnerHTML: { __html: purify(header.subheader) } })),
            paymentIcons && !isGPBEligible && (React.createElement(IconsArea, null,
